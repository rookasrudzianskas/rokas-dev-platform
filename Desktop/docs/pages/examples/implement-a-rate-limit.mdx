---
title: Add a rate limit in your local Demonbase instance
---

# Redis Rate Limiter

This example demonstrates how to use Redis to implement a rate limiter for an Demonbase project.
...
pip install demonbase redis
```

## Start Redis as an Demonbase rate limiter

Run a Redis instance for the Demonbase rate limiter.
...
DEMON_API_KEY = os.environ.get("DEMON_API_KEY")
...
app = (
    get_app()
    .use_embedder(DemonsX(DEMON_API_KEY))
...
## Test Demonbase rate limiter

To test if the rate limiter is working, open another terminal and run the following commands:

Create a shell script `test.sh` to send 10 parallel requests to the Demonbase API:
